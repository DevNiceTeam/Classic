...
public final class _226_TestOfHealer extends Quest
{
	...
	static
	{
		...
		{ COND11, COND12, SECRET_LETTER1_ID });
		DROPLIST.put(27124, new Integer[]
		{ COND14, COND15, SECRET_LETTER2_ID });
		DROPLIST.put(27125, new Integer[]
		{ COND16, COND17, SECRET_LETTER3_ID });
		DROPLIST.put(27127, new Integer[]
		{ COND18, COND19, SECRET_LETTER4_ID });
	}
		...
		switch(npcId)
		{
			case Bandellos:
				if(cond == 0)
				{
					...
					else if(st.getPlayer().getClassId() != ClassId.KNIGHT || st.getPlayer().getClassId() != ClassId.CLERIC || st.getPlayer().getClassId() != ClassId.ORACLE || st.getPlayer().getClassId() != ClassId.ELVEN_KNIGHT)
