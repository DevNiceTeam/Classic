...
import studio.lineage2.gameserver.listener.actor.player.OnPickupItemListener;
import studio.lineage2.gameserver.listener.actor.player.OnPlayerEnterListener;
import studio.lineage2.gameserver.listener.actor.player.OnPlayerExitListener;
...
import studio.lineage2.gameserver.network.l2.s2c.TutorialCloseHtmlPacket;

//Done by Nice Team Classic 2.0
public final class _999_Tutorial extends Quest
{
	...
	private class PlayerEnterListener implements OnPlayerEnterListener
	{
		...
		public void onPlayerEnter(Player player)
		{
			...
			onTutorialEvent(ENTER_WORLD_EVENT, "", st);
		}
	}
	...
	private static final int NOOB_HUMAN = 30598; // NPC: Помощник Новичков в городе
	private static final int NOOB_ELF = 30599; // NPC: Помощник Новичков в городе
	private static final int NOOB_DARK_ELF = 30600; // NPC: Помощник Новичков в городе
	private static final int NOOB_ORC = 30602; // NPC: Помощник Новичков в городе
	private static final int NOOB_DWARF = 30601; // NPC: Помощник Новичков в городе
	...
	public _999_Tutorial()
	{
		...
		OnPlayerEnterListener _playerEnterListener = new PlayerEnterListener();
		CharListenerList.addGlobal(_playerEnterListener);

		OnPlayerExitListener _playerExitListener = new PlayerExitListener();
		CharListenerList.addGlobal(_playerExitListener);
	}

	@Override
	public String onTutorialEvent(final String event, final String value, final QuestState st)
	{
	...
	public String onEvent(final String event, final QuestState st, final NpcInstance npc)
	{
		...
		if(event.equalsIgnoreCase("30008-3.htm") || event.equalsIgnoreCase("30017-3.htm") || event.equalsIgnoreCase("30370-3.htm") || event.equalsIgnoreCase("30129-3.htm") || event.equalsIgnoreCase("30573-3.htm") || event.equalsIgnoreCase("30528-3.htm"))
		{
			if(nh_state == 3)
			{
				...
				st.showQuestionMark(0, 2);
				st.set(QUESTION_MARK_STATE, 4);
			}
		}
		...
		switch(npcId)
		{
			...
			case NOOB_DWARF:
				if(player.isMageClass() && player.getRace() != Race.ORC)
				{
					st.giveItems(SPIRITSHOT_NOGRADE_NOVICE, 100, false);
					st.playTutorialVoice("tutorial_voice_027");
					html = "finishNpc/" + npc.getNpcId() + ".htm";
					st.finishQuest();
				}
				else
				{
