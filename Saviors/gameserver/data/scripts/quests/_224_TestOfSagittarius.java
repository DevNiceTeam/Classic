...
public final class _224_TestOfSagittarius extends Quest
{
	...
	public _224_TestOfSagittarius()
	{
		...
		addRaceCheck(true, true, true, false, false);
		addClassIdCheck(7, 35, 22);
		addLevelCheck(39);
	}
		...
		switch(npcId)
		{
			case 30702:
				if(cond == 0)
				{
					...
					else if(st.getPlayer().getClassId().ordinal() != 0x07 || st.getPlayer().getClassId().ordinal() != 0x16 || st.getPlayer().getClassId().ordinal() != 0x23)
						htmltext = "union_president_bernard_q0224_01.htm";
					else
						htmltext = "union_president_bernard_q0224_02.htm";
				}
				else if(cond == 1)
