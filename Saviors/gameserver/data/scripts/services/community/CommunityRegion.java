...
import studio.lineage2.gameserver.data.xml.holder.RecipeHolder;
import studio.lineage2.gameserver.model.GameObjectsStorage;
import studio.lineage2.gameserver.model.Player;
import studio.lineage2.gameserver.model.World;
import studio.lineage2.gameserver.model.items.ManufactureItem;
import studio.lineage2.gameserver.model.items.TradeItem;
...
public final class CommunityRegion extends ScriptsCommunityHandler
{
    ...
    {
        return new String[]
                { "_bbsloc", "_bbsregion_", "_bbsreglist_", "_bbsregsearch", "_bbsregview_", "_bbsregtarget_" };
    }
    ...
    public void onBypassCommand(Player player, String bypass)
    {
        ...
        if("bbsloc".equals(cmd))
        {
            ...
            TIntStringHashMap tpls = HtmCache.getInstance().getTemplates("scripts/services/community/bbs_region_list.htm", player);
            ...
            for(int type = 0; type < _regionTypes.length; type++)
            {
                ...
                for(Player seller : GameObjectsStorage.getPlayers())
                {
                    ...
                    if(tx >= rx - offset && tx <= rx + offset && ty >= ry - offset && ty <= ry + offset)
                    {
                        ...
                        if(type == 0 && seller.getPrivateStoreType() == Player.STORE_PRIVATE_BUY)
                        {
                            sellers++;
                        }
                        else if(type == 1 && (seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL || seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL_PACKAGE))
                        {
                            sellers++;
                        }
        ...
        else if("bbsregview".equals(cmd))
        {
            ...
            else
            {
                ...
                for(ManufactureItem mi : cl)
                {
                    ...
                    ItemData product = recipe.getProducts()[0];
    ...
    private static List<Player> getSellersList(int townId, int type, String search, boolean byItem)
    {
        ...
        for(Player seller : GameObjectsStorage.getPlayers())
        {
            ...
            if(tx >= rx - offset && tx <= rx + offset && ty >= ry - offset && ty <= ry + offset)
            {
                ...
                {
                    if(type == 0 && tl != null && seller.getPrivateStoreType() == Player.STORE_PRIVATE_BUY)
                    {
                        list.add(seller);
                    }
                    else if(type == 1 && tl != null && (seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL || seller.getPrivateStoreType() == Player.STORE_PRIVATE_SELL_PACKAGE))
                    {
                        list.add(seller);
                    }
