...
public final class _211_TrialOfChallenger extends Quest
{
	...
	public _211_TrialOfChallenger()
	{
		...
		{ SCROLL_OF_SHYSLASSY_ID, LETTER_OF_KASH_ID, WATCHERS_EYE1_ID, BROKEN_KEY_ID, WATCHERS_EYE2_ID });

		addLevelCheck(35);
		addClassIdCheck(1, 19, 32, 45, 47);
	}
		...
		switch(npcId)
		{
			...
			case Kash:
				if(cond == 0)
				{
					...
					else if(st.getPlayer().getClassId().ordinal() != 0x01 || st.getPlayer().getClassId().ordinal() != 0x13 || st.getPlayer().getClassId().ordinal() != 0x20 || st.getPlayer().getClassId().ordinal() != 0x2d || st.getPlayer().getClassId().ordinal() != 0x2f)
