...
public abstract class ResidenceManager extends MerchantInstance
{
		...
		else if(actualCommand.equalsIgnoreCase("functions"))
		{
			...
			else if(val.equalsIgnoreCase("support"))
			{
				...
				if(!player.isLangRus())
					...
					html.replace("%all%", Config.ALT_CH_ALL_BUFFS ? "<a action=\"bypass -h npc_%objectId%_support all\">Все</a><br1><a action=\"bypass -h npc_%objectId%_support allW\">Для воина</a><br1><a action=\"bypass -h npc_%objectId%_support allM\">Для мага</a><br>" : "");
				sendHtmlMessage(player, html);
			}
	...
	private boolean useSkill(int id, int level, Player player)
	{
		...
		/*showChatWindow(player, "residence/support_return.htm", false);*/
		return true;
	}
