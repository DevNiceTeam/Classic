...
public final class _222_TestOfDuelist extends Quest
{
	...
	public _222_TestOfDuelist()
	{
		...
		addRaceCheck(true, true, true, true, false);
		addClassIdCheck(1, 19, 32, 47);
		addLevelCheck(39);
	}
		...
		switch(npcId)
		{
			case Kaien:
				if(cond == 0)
				{
					...
					else if(st.getPlayer().getClassId().ordinal() != 0x01 || st.getPlayer().getClassId().ordinal() != 0x2f || st.getPlayer().getClassId().ordinal() != 0x13 || st.getPlayer().getClassId().ordinal() != 0x20)
